...
public class LoanRecoveryService {
    ...
    public LoanRecoveryDetails getLoanRecoveryDetails(String loanNumber)
            throws SQLException, LoanNotFoundException, UnauthorizedLoanException {
        try {
            ...
            if (currentLoanStatus == LOAN_STATUS_CANCELLED) {
                throw new UnauthorizedLoanException(
                        "Loan Number: " + loanNumber + " is cancelled and cannot be recovered.");
            }
            if (currentLoanStatus == LOAN_STATUS_UNAUTHORIZED) {
                throw new UnauthorizedLoanException(
                        "Loan Number: " + loanNumber + " is unauthorized and cannot be recovered.");
            }
            ...
            if (details == null) {
                throw new LoanNotFoundException("Loan recovery details not found for Loan Number: " + loanNumber
                        + ". Check if loan exists and has associated account/currency/financial data.");
            }
            return details;
    ...
    public LoanRecoveryDetails performLoanRecovery(String loanNumber, String recoveredByUserId)
            throws SQLException, LoanNotFoundException, RecoveryAlreadyExistsException, UnauthorizedLoanException {
        ...
        if (currentLoanStatus != LOAN_STATUS_READY_FOR_RECOVERY) {
            throw new UnauthorizedLoanException("Loan Number: " + loanNumber
                    + " is not ready for recovery (current status: " + currentLoanStatus + ").");
        }

        // 2. Check if recovery already exists
        if (loanRecoveryRepository.doesRecoveryExist(loanNumber)) {
            throw new RecoveryAlreadyExistsException(
                    "Loan recovery record already exists for Loan Number: " + loanNumber);
        }
        ...
        if (originalGrantDate == null) {
            throw new LoanNotFoundException(
                    "Original grant date could not be retrieved for Loan Number: " + loanNumber);
        }
    ...
    public void sendToAuthorizationQueue(String loanNumber)
            throws SQLException, LoanNotFoundException, UnauthorizedLoanException {
        Integer currentLoanStatus = loanApplicationRepository.getStatusByLoanNumber(loanNumber);
        if (currentLoanStatus == null) {
            throw new LoanNotFoundException("Loan not found with Loan Number: " + loanNumber);
        ...
        if (currentLoanStatus != LOAN_STATUS_RECOVERY_PENDING_AUTHORIZATION) {
            throw new UnauthorizedLoanException("Loan Number: " + loanNumber
                    + " is not in a state where it can be sent for recovery authorization (current status: "
                    + currentLoanStatus + ").");
        }

        if (!loanRecoveryRepository.doesRecoveryExist(loanNumber)) {
            throw new LoanNotFoundException(
                    "No recovery record found for Loan Number: " + loanNumber + ". Please perform recovery first.");
        }

        loanRecoveryRepository.updateRecoveryStatus(loanNumber, 6); // Set LoanRecovery.Status to 6 (Authorization
        // Pending)
        // LoanApplication status is already 7 (Recovery Pending Authorization)
    }

    @Transactional(rollbackOn = Exception.class)
    public void authorizeRecovery(String loanNumber, String authorizedByUserId)
            throws SQLException, LoanNotFoundException, UnauthorizedLoanException {
        Integer currentLoanStatus = loanApplicationRepository.getStatusByLoanNumber(loanNumber);
        if (currentLoanStatus == null) {
            throw new LoanNotFoundException("Loan not found with Loan Number: " + loanNumber);
        ...
        if (currentLoanStatus != LOAN_STATUS_RECOVERY_PENDING_AUTHORIZATION && currentLoanStatus != 10) {
            throw new UnauthorizedLoanException("Loan Number: " + loanNumber
                    + " is not pending recovery authorization (current status: " + currentLoanStatus + ").");
        }
        ...
        // Credit back the deal amount to customer's limit balance >>>
        if (customerNumber != null && dealAmount != null && dealAmount.compareTo(BigDecimal.ZERO) > 0) {
            // The call to creditBackDealAmount is void and will throw an exception on
            // failure.
            // This exception will be caught by the @Transactional annotation on
            // authorizeRecovery
            // and trigger a rollback if not handled explicitly here.
            try {
                loanLimitService.creditBackDealAmount(customerNumber, dealAmount, authorizedByUserId);
                System.out.println(
                        "Successfully credited back deal amount: " + dealAmount + " to customer: " + customerNumber);
            } catch (RuntimeException e) { // Catch specific runtime exceptions if needed, or let them propagate
                System.err.println("Critical error during limit balance update for customer " + customerNumber + ": "
                        + e.getMessage());
                // Re-throw to ensure the main transaction rolls back
                throw e;
            }
        } else {
            String errorMsg = "Could not credit back deal amount: customerNumber or dealAmount is null/invalid for loan: "
                    + loanNumber;
            System.err.println(errorMsg);
            // This situation should ideally also cause a rollback if it's an inconsistent
            // state.
    ...
    public void rejectRecovery(String loanNumber)
            throws SQLException, LoanNotFoundException, UnauthorizedLoanException {
        Integer currentLoanStatus = loanApplicationRepository.getStatusByLoanNumber(loanNumber);
        if (currentLoanStatus == null) {
            throw new LoanNotFoundException("Loan not found with Loan Number: " + loanNumber);
